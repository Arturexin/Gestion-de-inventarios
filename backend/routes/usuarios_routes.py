...
usuario_db = Blueprint('usuario_db', __name__)
...
def getUsuario(id):
    try:
        with mysql.connection.cursor() as cur:
            query = ("SELECT id, num_sucursales, num_usuarios, nombres, cargo FROM usuarios "
                     "WHERE id = %s ")
...
@usuario_db.route('/api/usuario_db/<int:id>')#Index
@cross_origin()
# @login_required
def getUsuarioDB(id):
    try:
        with mysql.connection.cursor() as cur:
            query = ("SELECT id, nombres, apellidos, dni, e_mail, cargo FROM usuarios "
                     "WHERE id = %s ")
            cur.execute(query, (id,))
            data = cur.fetchall()
        contenido = {}
