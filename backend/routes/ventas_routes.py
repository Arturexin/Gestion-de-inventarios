...
def getAllVentas(numero):
    try:
        ...
        with mysql.connection.cursor() as cur:
            query = ("SELECT id_det_ventas, sucursal_nombre, comprobante, tipo_comprobante, nombre_cli, modo_efectivo, modo_credito, modo_tarjeta, modo_perdida, fecha_det_ventas, canal_venta, situacion, id_sucursales "
                        "FROM ventas "
                        "JOIN sucursales ON `ventas`.`sucursal` = `sucursales`.`id_sucursales` "
                        "JOIN clientes ON `ventas`.`dni_cliente` = `clientes`.`id_cli` "
                        "WHERE identificador_ventas = %s "
        ...
        for fila in data:
            contenido = { 
                ...
                'nombre_cli': fila[4],
                'modo_efectivo': formatear_moneda(fila[5]),
                'modo_credito': formatear_moneda(fila[6]),
                'modo_tarjeta': formatear_moneda(fila[7]),
                'modo_perdida': formatear_moneda(fila[8]),
                'fecha_det_ventas': fila[9].strftime('%d-%m-%Y'),
                'canal_venta': 'Delivery' if fila[10] > 0 else 'Local',
                'situacion': fila[11],
                'id_sucursales': fila[12],
                }
