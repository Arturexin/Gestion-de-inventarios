...
procesar_devolucion_entradas_post = Blueprint('procesar_devolucion_entradas_post', __name__)
...
def getAllEntradas(numero):
    try:
        ...
        with mysql.connection.cursor() as cur:
            query = ("SELECT idEntr, sucursal_nombre, categoria_nombre, codigo, costo_unitario, existencias_entradas, comprobante, fecha, existencias_devueltas, entradas.idProd, id_sucursales, descripcion "
                        "FROM entradas "
                        "JOIN almacen_central ON `entradas`.`idProd` = `almacen_central`.`idProd` "
                        "JOIN categorias ON `almacen_central`.`categoria` = `categorias`.`id` "
                        "JOIN sucursales ON `entradas`.`sucursal` = `sucursales`.`id_sucursales` "
        ...
        for fila in data:
            contenido = { 
                ...
                'codigo': fila[3],
                'costo_unitario': formatear_moneda(fila[4]),
                'existencias_operacion':fila[5],
                'comprobante': fila[6],
                'fecha': fila[7].strftime('%d-%m-%Y'),
                'existencias_devueltas': fila[8],
                'total_costo': formatear_moneda((fila[5] - fila[8]) * fila[4]),
                'idProd': fila[9],
                'id_sucursales': fila[10],
                'descripcion': fila[11],
                }
...
@procesar_devolucion_entradas_post.route('/api/procesar_devolucion_compras', methods=['POST'])##Devolución por compras y Productos
@cross_origin()
# @login_required
