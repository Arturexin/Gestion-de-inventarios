...
def getAllSalidas(numero):
    try:
        ...
        with mysql.connection.cursor() as cur:
            query = ("SELECT idSal, sucursal_nombre, categoria_nombre, codigo, existencias_salidas, precio_venta_salidas, comprobante, fecha, existencias_devueltas, cliente, salidas.idProd, id_sucursales, descripcion  "
                        "FROM salidas "
                        "JOIN almacen_central ON `salidas`.`idProd` = `almacen_central`.`idProd` "
                        "JOIN categorias ON `almacen_central`.`categoria` = `categorias`.`id` "
                        "JOIN sucursales ON `salidas`.`sucursal` = `sucursales`.`id_sucursales` "
        ...
        for fila in data:
            contenido = { 
                ...
                'codigo': fila[3],
                'existencias_operacion': fila[4],
                'precio_venta_salidas': formatear_moneda(fila[5]),
                'comprobante': fila[6],
                'fecha': fila[7].strftime('%d-%m-%Y'),
                'existencias_devueltas': fila[8],
                'cliente': fila[9],
                'total_venta': formatear_moneda((fila[4] - fila[8]) * fila[5]),
                'idProd': fila[10],
                'id_sucursales': fila[11],
                'descripcion': fila[12],
                }
        ...
        with mysql.connection.cursor() as cur:
            ...
            for fila in data:
                contenido = {
                    ...
                    'sucursal': fila[1],
                    'categoria': fila[2],
                    'nombre': fila[3],  # Usar el código base
                    'suma_ventas': fila[4],
                    'suma_costos': fila[5],
                    'suma_unidades': int(fila[6]),
...
def getCodigoSucursalSalidas(codigo):
    try:
        ...
        with mysql.connection.cursor() as cur:
            query = ("SELECT MONTH(fecha) AS mes, sucursal, categoria, codigo, "
                     "SUM((existencias_salidas - existencias_devueltas) * precio_venta_salidas) AS suma_ventas, "
                     "SUM((existencias_salidas - existencias_devueltas) * costo_unitario) AS suma_costos, "
                     "SUM(existencias_salidas - existencias_devueltas) AS suma_unidades, "
                     "SUM(existencias_devueltas) AS suma_unidades_dev, "
